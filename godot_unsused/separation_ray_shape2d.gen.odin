...
Separation_Ray_Shape2d_Constants :: enum {
}



separation_ray_shape2d_name_ref :: proc "contextless" () -> ^String_Name {
    return &__class_name
}

separation_ray_shape2d_name :: proc "contextless" () -> String_Name {
    return __class_name
}

new_separation_ray_shape2d :: proc "contextless" () -> Separation_Ray_Shape2d {
    return cast(Separation_Ray_Shape2d)__bindgen_gde.classdb_construct_object(separation_ray_shape2d_name_ref())
}

// methods

separation_ray_shape2d_set_length :: proc "contextless" (
    self: Separation_Ray_Shape2d,
    length_: f32,
) {
    self := self
    length_ := length_
...
separation_ray_shape2d_get_length :: proc "contextless" (
    self: Separation_Ray_Shape2d,
) -> (ret: f32) {
    self := self
    args := []__bindgen_gde.TypePtr {
...
separation_ray_shape2d_set_slide_on_slope :: proc "contextless" (
    self: Separation_Ray_Shape2d,
    active_: Bool,
) {
    self := self
...
separation_ray_shape2d_get_slide_on_slope :: proc "contextless" (
    self: Separation_Ray_Shape2d,
) -> (ret: Bool) {
    self := self
    args := []__bindgen_gde.TypePtr {
...
separation_ray_shape2d_init :: proc "contextless" () {
    __class_name = new_string_name_cstring("SeparationRayShape2D", true)
    __name: String_Name
