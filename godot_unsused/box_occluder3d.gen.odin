...
Box_Occluder3d_Constants :: enum {
}



box_occluder3d_name_ref :: proc "contextless" () -> ^String_Name {
    return &__class_name
}

box_occluder3d_name :: proc "contextless" () -> String_Name {
    return __class_name
}

new_box_occluder3d :: proc "contextless" () -> Box_Occluder3d {
    return cast(Box_Occluder3d)__bindgen_gde.classdb_construct_object(box_occluder3d_name_ref())
}

// methods

box_occluder3d_set_size :: proc "contextless" (
    self: Box_Occluder3d,
    size_: Vector3,
) {
    self := self
...
box_occluder3d_get_size :: proc "contextless" (
    self: Box_Occluder3d,
) -> (ret: Vector3) {
    self := self
    args := []__bindgen_gde.TypePtr {
...
box_occluder3d_init :: proc "contextless" () {
    __class_name = new_string_name_cstring("BoxOccluder3D", true)
    __name: String_Name
