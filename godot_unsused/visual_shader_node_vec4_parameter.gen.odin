...
Visual_Shader_Node_Vec4_Parameter_Constants :: enum {
}



visual_shader_node_vec4_parameter_name_ref :: proc "contextless" () -> ^String_Name {
    return &__class_name
}

visual_shader_node_vec4_parameter_name :: proc "contextless" () -> String_Name {
    return __class_name
}

new_visual_shader_node_vec4_parameter :: proc "contextless" () -> Visual_Shader_Node_Vec4_Parameter {
    return cast(Visual_Shader_Node_Vec4_Parameter)__bindgen_gde.classdb_construct_object(visual_shader_node_vec4_parameter_name_ref())
}

// methods

visual_shader_node_vec4_parameter_set_default_value_enabled :: proc "contextless" (
    self: Visual_Shader_Node_Vec4_Parameter,
    enabled_: Bool,
) {
    self := self
    enabled_ := enabled_
...
visual_shader_node_vec4_parameter_is_default_value_enabled :: proc "contextless" (
    self: Visual_Shader_Node_Vec4_Parameter,
) -> (ret: Bool) {
    self := self
    args := []__bindgen_gde.TypePtr {
...
visual_shader_node_vec4_parameter_set_default_value :: proc "contextless" (
    self: Visual_Shader_Node_Vec4_Parameter,
    value_: Vector4,
) {
    self := self
...
visual_shader_node_vec4_parameter_get_default_value :: proc "contextless" (
    self: Visual_Shader_Node_Vec4_Parameter,
) -> (ret: Vector4) {
    self := self
    args := []__bindgen_gde.TypePtr {
...
visual_shader_node_vec4_parameter_init :: proc "contextless" () {
    __class_name = new_string_name_cstring("VisualShaderNodeVec4Parameter", true)
    __name: String_Name
    ...
    __set_default_value_method_ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &__name, 643568085)
    __name = new_string_name_cstring("get_default_value", true)
    __get_default_value_method_ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &__name, 2435802345)
}
